...
bool cxa_mqtt_rpc_message_isActionableRequest(cxa_mqtt_message_t *const msgIn, char** methodNameOut, size_t* methodNameLen_bytesOut, char** idOut, size_t* idLen_bytesOut)
{
	...
	ssize_t methodIndex = cxa_stringUtils_indexOf_withLengths(topic, topicLen_bytes, CXA_MQTT_RPCNODE_REQ_PREFIX, strlen(CXA_MQTT_RPCNODE_REQ_PREFIX));
	...
	ssize_t separatorIndex = cxa_stringUtils_indexOf_withLengths(&topic[methodIndex], (topicLen_bytes - methodIndex), "/", 1);
...
bool cxa_mqtt_rpc_message_isActionableResponse(cxa_mqtt_message_t *const msgIn, char** methodNameOut, size_t* methodNameLen_bytesOut, char** idOut, size_t* idLen_bytesOut)
{
	...
	ssize_t methodIndex = cxa_stringUtils_indexOf_withLengths(topic, topicLen_bytes, CXA_MQTT_RPCNODE_RESP_PREFIX, strlen(CXA_MQTT_RPCNODE_RESP_PREFIX));
	if( methodIndex < 0 ) return false;
	methodIndex += 2;
