...
bool cxa_fixedFifo_queue(cxa_fixedFifo_t *const fifoIn, void *const elemIn)
{
	...
	fifoIn->insertIndex = ((fifoIn->insertIndex+1) >= fifoIn->maxNumElements) ? 0 : fifoIn->insertIndex+1;
...
bool cxa_fixedFifo_dequeue(cxa_fixedFifo_t *const fifoIn, void *elemOut)
{
	...
	fifoIn->removeIndex = ((fifoIn->removeIndex+1) >= fifoIn->maxNumElements) ? 0 : fifoIn->removeIndex+1;
...
size_t cxa_fixedFifo_getCurrSize(cxa_fixedFifo_t *const fifoIn)
{
	...
	size_t idx_insert = fifoIn->insertIndex;
	size_t idx_remove = fifoIn->removeIndex;

	return (idx_insert >= idx_remove) ?
		(idx_insert - idx_remove) :
		((fifoIn->maxNumElements-idx_remove) + idx_insert);
}
...
bool cxa_fixedFifo_isFull(cxa_fixedFifo_t *const fifoIn)
{
	...
	return (idx_remove != 0) ?
		((idx_remove-1) == idx_insert) :
		(idx_insert == (fifoIn->maxNumElements-1));
}
