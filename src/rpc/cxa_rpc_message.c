...
bool cxa_rpc_message_validateReceivedBytes(cxa_rpc_message_t *const msgIn)
{
	cxa_assert(msgIn);
	...
	if( !cxa_linkedField_initRoot_fixedLen(&msgIn->type, msgIn->buffer, 0, 1) ) { msgIn->areFieldsConfigured = false; return false; }
	...
	switch( msgType )
	{
		case CXA_RPC_MESSAGE_TYPE_REQUEST:
		{
			...
			// finally, our params
			if( !cxa_linkedField_initChild(&msgIn->params, &msgIn->id, (cxa_fixedByteBuffer_getSize_bytes(msgIn->buffer) - cxa_linkedField_getStartIndexOfNextField(&msgIn->id))) ) { msgIn->areFieldsConfigured = false; return false; }
		...
		case CXA_RPC_MESSAGE_TYPE_RESPONSE:
		{
			...
			// finally, our params
			if( !cxa_linkedField_initChild(&msgIn->params, &msgIn->returnValue, (cxa_fixedByteBuffer_getSize_bytes(msgIn->buffer) - cxa_linkedField_getStartIndexOfNextField(&msgIn->returnValue))) ) { msgIn->areFieldsConfigured = false; return false; }
